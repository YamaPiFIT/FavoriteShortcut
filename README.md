...
- **Edge / Chrome のお気に入りの取り込み**（フォルダ構成ごと）
- エクスプローラーからのドラッグ＆ドロップ登録（複数まとめて登録も可）
- ショートカットをドラッグしてフォルダ間を移動
- アイコン: Web は favicon を自動取得（取れない場合はブラウザのキャッシュから）、
  ファイル / フォルダは Windows のアイコン、任意の画像も指定可
...
すでに Edge や Chrome に貯めてあるお気に入りを、まとめて取り込めます。

`⋯` →「ブラウザのお気に入りを取り込む...」

1. **取り込み元のブラウザ**を選びます
   （Microsoft Edge / Google Chrome。Brave / Vivaldi / Opera / Chromium も
   同じ形式なのでインストールされていれば選べます。複数プロファイルがある場合は
   ブラウザ側で付けた名前で区別できます）
2. **取り込む範囲**を選びます（お気に入りバー / その他 / モバイル）
3. **取り込み先**を決めます（新しいフォルダを作る / 既存のフォルダへ）
4. 必要に応じてオプションを設定します
   - ブラウザのフォルダ構成を再現する
   - すでに登録済みの URL は取り込まない
   - 取り込んだ項目にタグを付ける（例: `Chrome`）

ブラウザのファイルは**読み取るだけ**で、お気に入り側には一切変更を加えません。
ブラウザを起動したままでも取り込めます。

> ブックマークレット（`javascript:`）やブラウザ内部ページ（`chrome://` `edge://`）は、
> ショートカットとして開けないため取り込み対象から除外されます。

アイコンは取り込み後に順次取得されます（そのブラウザがアイコンを持っているため、
ほとんどの項目ですぐに表示されます）。
...
| 区分 | 内容 |
|---|---|
| 種別判定 | Web / フォルダ / ファイル / EXE / UNC / 存在しないパス / URL 正規化 / オリジン |
| アイコン | ブラウザのキャッシュ読み取り（Chromium / Firefox 形式）、http と https の区別、SVG の除外、LIKE のエスケープ |
| お気に入り取り込み | Bookmarks ファイルの解析、階層の再帰、javascript: / chrome:// の除外、追加日時、壊れたファイルの扱い |
...
| 種別 | 方法 |
|---|---|
| Web | ① 登録された URL から直接取得 → ② ブラウザのアイコンキャッシュ の順 |
...
favicon は `iconsav_<オリジンのハッシュ>.png` としてキャッシュするので、
**オフラインでも表示できます**。同じサイトのショートカット同士でキャッシュを共有します。

#### ① サイトから直接取得

「登録された URL そのもの」を起点に、HTML の `<link rel="icon">` → サブパス直下 →
サイトルートの `/favicon.ico` の順に探します。ホスト名だけから `https://host/` を作り直すと、
http のみのサイト・非標準ポート・サブパスに置かれた社内システムを取りこぼすためです。

#### ② ブラウザのアイコンキャッシュ（既定で有効）

直接取得できなかった場合、ブラウザが保存しているアイコンを探します。
**Chrome / Edge / Brave / Vivaldi / Opera / Chromium / Firefox** に対応しています。

これにより、次のようなサイトのアイコンも表示できます。

- bot 対策で HTTP アクセスが拒否されるサイト（`403` を返すもの）
- ログインや SSO が必要な社内サイト
- JavaScript で描画されるサイト、SVG しか配信していないサイト

**一度ブラウザで開けばアイコンが付きます。** ショートカットを起動すると、
ブラウザ側が favicon を保存するのを待って自動的に取り直します。

取り扱いは次のとおりです。

- **通信は一切行いません**。ローカルファイルを読み取り専用で開くだけです
- **認証情報は扱いません**
- 登録済みショートカットの URL でピンポイントに検索します。一覧の走査はしません
- 取り出すのは一致したアイコン画像のみで、URL や閲覧履歴は保持しません
- ブラウザのファイルには一切書き込みません（一時コピーを読み、20 秒以内に削除します）
- ブラウザがダークモード用の白いアイコンを保存している場合、
  背景に埋もれて見えなくなるため採用せず、既定アイコンを表示します

不要であれば 設定 → アイコン から無効にできます。

### 採用しなかった方式

**外部のアイコン取得サービス**（Google の favicon API など）は使いません。
ドメイン名が外部へ送信されるうえ、社内サイトには到達できないため効果がありません。
ブラウザのキャッシュを参照すれば同じ目的をローカルだけで達成できるので、
**このアプリが利用者の知らない第三者と通信することはありません**。

**Windows 統合認証による favicon 取得**は、社内サイトの 401 を解決できる一方で採用しませんでした。
ブラウザが認証情報を送るのはユーザーが自分でアクセスした瞬間だけですが、このアプリが同じことを
すると「登録されているだけで裏で自動的に認証付きリクエストを投げる」動作になります。
ドメイン非参加の PC では単一ラベル名が自動的にイントラネットゾーンに含まれるため、
社外ネットワーク上の悪意あるホストへ NetNTLMv2 ハッシュを渡してしまう恐れがあり、
「アイコンを表示する」という利益に見合いません。
社内サイトのアイコンは ② のブラウザキャッシュで取得できます。
