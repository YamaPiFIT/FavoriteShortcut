...
| 種別 | 方法 |
|---|---|
| Web | 登録された URL の HTML から `<link rel="icon">` を解析 → サブパス直下 → サイトルートの `/favicon.ico` の順に試行 |
...
取得先は「登録された URL そのもの」を起点に組み立てます。ホスト名だけから `https://host/` を
作り直すと、http のみのサイト・非標準ポート・サブパスに置かれた社内システムを取りこぼすためです。

外部のアイコン取得サービス（Google の favicon API）は**既定で無効**です。
有効にするとドメイン名が外部へ送信されるため、設定画面で明示的にオンにしたときだけ使います。
さらに、有効にした場合でも次のホストは**外部サービスへ問い合わせません**（社内の命名規則が
外部へ漏れるのを防ぐため）。

- ドットを含まない単一ラベル名（`http://portal/` など）
- `.local` / `.internal` / `.corp` / `.lan` などの社内向けサフィックス
- プライベート IP アドレス（10/8、172.16/12、192.168/16、127/8、169.254/16、IPv6 ULA）
...
認証が必要なサイトのアイコンは、登録画面の「画像を選択...」から任意の画像を指定してください。
